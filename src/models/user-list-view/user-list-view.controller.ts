...
export class UserListViewController {
  ...
  @UseGuards(JwtAuthGuard)
  @Delete(":id/:videoId")
  deleteVideoWithListView(@Param("id", ParseIntPipe) id: number,
                          @Param("videoId", ParseIntPipe) videoId: number, @Req() req) {
    return this.userListViewService.delete(videoId, id, req.user);
  }
}
